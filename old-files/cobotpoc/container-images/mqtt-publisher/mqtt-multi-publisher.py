...
LOOP_PERIOD_S   = float(os.environ.get("LOOP_PERIOD_S", "20.0"))
...
# Fixed (X, Y) waypoints (mm) to cycle through on each publish tick. Mix of
# safe and unsafe poses (per `_isSafe`) so subscribers see both states.
FIXED_XY_COORDS = [
    (-125, 357),
    (164,  262),
    (66,   138),
...
def main() -> int:
    ...
    sequence = 0
    print(
        f"publishing to {MQTT_HEALTH}, {MQTT_COORDS}, {MQTT_ROBOTBUSY} every 10-60s "
        f"(qos={MQTT_QOS}, leader_id={LEADER_ID}). Ctrl+C to stop."
    )
    try:
        while not stoppingEvent.is_set():
            # Cycle through the fixed XY waypoints; Z and Rz stay random.
            inputX, inputY = FIXED_XY_COORDS[sequence % len(FIXED_XY_COORDS)]
            inputZ  = round(random.uniform(*COORD_Z_RANGE),  2)
            inputRz = round(random.uniform(*COORD_RZ_RANGE), 2)
            coordsPayload = {
                "timestamp": datetime.now(timezone.utc).isoformat(),
                ...
                "sequence":  sequence,
                "coords": {
                    "inputX":   round(float(inputX), 2),
                    "inputY":   round(float(inputY), 2),
                    "inputZ":   round(float(inputZ), 2),
                    "inputRz":  round(float(inputRz), 2),
                    "safe":    _isSafe(inputX, inputY),
                },
                "period_s": LOOP_PERIOD_S,
            }
            ...
            sequence += 1
            # Sleep a random 10-60s, but wake immediately if Ctrl+C / SIGTERM
            # arrives. wait() returns True once the event is set.
            period = random.uniform(10.0, 60.0)
            print(f"sleeping {period:.1f}s before next tick", flush=True)
